...
export const signin = async (req, res) => {
    ...
    try {
        ...
        if (existingUser === null) { return res.status(404).json({ message: 'User doesn`t exist' }) }

        const isPasswordCorrect = await bcrypt.compare(password, existingUser.password)

        if (!isPasswordCorrect) { return res.status(400).json({ message: 'Invalid credentionals' }) }
